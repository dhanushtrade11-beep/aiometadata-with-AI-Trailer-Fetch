...
// ─── IDLookup ─────────────────────────────────────────────────────────────────
function IDLookupPanel({ type, onFound }: {
  type: "movie" | "series";
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      if (res.status === 401) throw new Error("Unauthorized — check your admin key in Settings");
      if (res.status === 404) {
        const data = await res.json().catch(() => ({}));
        // 404 could mean route not found (old backend) or title not found on TMDB
        if (data.error === "Not found on TMDB") throw new Error("Not found on TMDB — double-check the ID");
        throw new Error("Lookup route not found — make sure mediaManagerRoutes.js is updated on the server");
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/media-manager/override/${selected.id}`, {
        ...
        body: JSON.stringify({ override: payload, type: selected.type, imdbId: selected.imdb_id || null }),
      });
      const data = await res.json();
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {/* ── ID Lookup fallback ── */}
          <IDLookupPanel type={type} onFound={handleIDFound} />
        </CardContent>
      </Card>
