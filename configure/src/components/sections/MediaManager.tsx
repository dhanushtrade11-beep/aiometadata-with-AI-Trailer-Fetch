...
import {
  ...
  ExternalLink, RefreshCw, Trash2, Check, AlertCircle, Play
} from "lucide-react";
...
// ─── TrailerFixField ──────────────────────────────────────────────────────────
// Prominent trailer fix UI shown directly in the Edit Details panel
function TrailerFixField({
  trailerInput,
  setTrailerInput,
  existing,
  aiGenerated,
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        {existing?.trailerYtId && !isChanged && trailerInput === "" && (
          <Badge variant="outline" className="text-xs border-green-500/40 text-green-400 bg-green-500/10 flex items-center gap-1">
            ...
            Custom trailer saved
          </Badge>
        )}
      </div>

      {/* AI-generated trailer warning */}
      {hasAiTrailer && (
        <div className="flex items-start gap-2 bg-amber-500/8 border border-amber-500/20 rounded-lg p-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              If this trailer is wrong, paste the correct YouTube link below to override it.
            </p>
            <div className="flex items-center gap-2 mt-1.5">
              <a
                href={`https://www.youtube.com/watch?v=${aiGenerated}`}
                target="_blank"
                rel="noreferrer"
              ...
              <button
                onClick={() => window.open(`https://www.youtube.com/watch?v=${aiGenerated}`, "_blank")}
      ...
      <div className="flex gap-2">
        <Input
          ...
          placeholder="Paste correct YouTube URL or video ID..."
      ...
      <p className="text-xs text-muted-foreground">
        Accepts full YouTube URLs, youtu.be links, or plain 11-character video IDs.
      </p>

      {/* Live preview of new trailer */}
      {trailerInput && ytId && (
        <div className="space-y-1.5">
          <p className="text-xs font-medium text-green-400 flex items-center gap-1">
            <Check className="h-3 w-3" /> Valid — preview below
          </p>
      ...
      {trailerInput && !ytId && (
        <p className="text-xs text-red-400 flex items-center gap-1">
          <AlertCircle className="h-3 w-3" /> Not a valid YouTube URL or video ID
        </p>
      )}
    </div>
  );
...
export function MediaManager() {
  ...
  // AI-fetched trailer from metadata (separate from override)
  ...
  const handleSelect = async (r: SearchResult) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Store AI-fetched trailer if available from metadata
        if (data.aiTrailerYtId) {
          setAiTrailerYtId(data.aiTrailerYtId);
        }
      }
    } catch {
        ...
        <Card className="border-blue-500/30">
          ...
          <CardContent>
            {loadingOverride ? (
              ...
              <div className="space-y-6">
                {/* ── Edit Details: Poster + Trailer side-by-side ── */}
                <div className="flex gap-5">
                  ...
                  <div className="shrink-0 space-y-2">
                    ...
                    {(override.poster || selected.poster) && (
                      <button
                        onClick={() => window.open(override.poster || selected.poster || "", "_blank")}
                  ...
                  <div className="flex-1 min-w-0">
                    <TrailerFixField
                      ...
                      setTrailerInput={setTrailerInput}
                      existing={existing}
                      aiGenerated={aiTrailerYtId}
                    />
                  </div>
                </div>

                {/* ── Tabs for the rest of the fields ── */}
