...
export function AdminProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const checkExistingSession = async () => {
      try {
        ...
        const guestSession = localStorage.getItem(GUEST_SESSION_STORAGE);
        ...
        const sessionActive = localStorage.getItem(ADMIN_SESSION_STORAGE);
        ...
        localStorage.removeItem(ADMIN_KEY_STORAGE);
        localStorage.removeItem(ADMIN_SESSION_STORAGE);
        localStorage.removeItem(GUEST_SESSION_STORAGE);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = async (key: string): Promise<boolean> => {
    try {
      ...
      if (isValid) {
        ...
        localStorage.setItem(ADMIN_SESSION_STORAGE, 'true');
        return true;
      } else {
        return false;
      }
  ...
  const loginAsGuest = () => {
    ...
    setIsAdmin(false);
    setAdminKey(null);
    localStorage.setItem(GUEST_SESSION_STORAGE, 'true');
