...
function registerMediaManagerRoutes(addon, redis) {
  ...
  addon.post('/api/media-manager/override/:id', async (req, res) => {
    ...
    // Save under the primary id (e.g. tmdb:1367220)
    ...
    // Also save under imdb_id if provided, so catalog lookup (which uses tt id) finds it too
    const imdbId = req.body && req.body.imdbId;
    if (imdbId && imdbId.startsWith('tt')) {
      await redisSet(redis, `${OVERRIDE_PREFIX}${imdbId}`, clean).catch(() => {});
