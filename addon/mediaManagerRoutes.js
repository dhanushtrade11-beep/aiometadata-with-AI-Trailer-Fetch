...
async function invalidateTrailerCache(redis, stremioId) {
  try {
    if (!redis) return;
    for (const v of ['v7','v8','v9','v10']) {
      await redis.del(`trailer:${v}:${stremioId}`).catch(() => {});
    }
  } catch {}
}
...
function registerMediaManagerRoutes(addon, redis) {
  ...
  addon.post('/api/media-manager/override/:id', async (req, res) => {
    ...
    if (clean.trailerYtId) await invalidateTrailerCache(redis, id);
