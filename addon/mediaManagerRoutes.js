...
async function redisSet(redis, key, value) {
  try {
    ...
    await redis.set(key, JSON.stringify(value), 'EX', OVERRIDE_TTL);
    return true;
  } catch { return false; }
}
...
function registerMediaManagerRoutes(addon, redis) {
  ...
  addon.post('/api/media-manager/override/:id', async (req, res) => {
    ...
    // Invalidate ALL meta cache keys for this movie/series
    // Meta keys follow pattern: meta:<cfgHash>:<metaId> — we scan for all variants
    try {
      // The stremioId might be tmdb:XXXX or tt... — try both
      const idsToInvalidate = new Set([id]);
      // Also try stripping the tmdb: prefix to get numeric id
      if (id.startsWith('tmdb:')) idsToInvalidate.add(id.replace('tmdb:', ''));

      let totalDeleted = 0;
      for (const searchId of idsToInvalidate) {
        // Use SCAN to find keys matching meta:*:<searchId>
        let cursor = '0';
        do {
          const [nextCursor, keys] = await redis.scan(cursor, 'MATCH', `meta:*:*${searchId}*`, 'COUNT', 100);
          cursor = nextCursor;
      ...
      if (totalDeleted > 0) {
        console.log(`[MediaManager] Cleared ${totalDeleted} meta cache key(s) for ${id}`);
      }
    } catch (e) {
      console.log(`[MediaManager] Meta cache clear note: ${e.message}`);
    }
