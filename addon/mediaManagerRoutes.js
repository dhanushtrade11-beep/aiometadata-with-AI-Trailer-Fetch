...
function registerMediaManagerRoutes(addon, redis) {
  ...
  addon.post('/api/media-manager/override/:id', async (req, res) => {
    ...
    // Invalidate trailer cache under ALL possible key formats
    await invalidateTrailerCache(redis, id);

    // Also invalidate meta cache so the override is applied on next request
    // Meta cache keys follow pattern: *:<id>* — delete by scanning
    try {
      const metaKeys = await redis.keys(`*meta*${id}*`);
      if (metaKeys && metaKeys.length > 0) {
        await redis.del(...metaKeys);
        console.log(`[MediaManager] Invalidated ${metaKeys.length} meta cache key(s) for ${id}`);
      }
    } catch {}
