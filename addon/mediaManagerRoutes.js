...
function registerMediaManagerRoutes(addon, redis) {
  ...
  addon.post('/api/media-manager/override/:id', async (req, res) => {
    ...
    // Invalidate ALL cache keys for this movie/series
    // Patterns: meta:*, meta-poster:*, meta-basic:*, meta-background:*, meta-logo:*, meta-trailers:*, etc.
    try {
      const idsToInvalidate = new Set([id]);
      if (id.startsWith('tmdb:')) idsToInvalidate.add(id.replace('tmdb:', ''));

      // All component cache prefixes used by cacheWrapMetaSmart
      ...
      for (const searchId of idsToInvalidate) {
        for (const prefix of CACHE_PREFIXES) {
          let cursor = '0';
          do {
            const [nextCursor, keys] = await redis.scan(cursor, 'MATCH', prefix + '*:' + searchId, 'COUNT', 200);
            cursor = nextCursor;
      ...
      if (totalDeleted > 0) {
        console.log('[MediaManager] Cleared ' + totalDeleted + ' cache key(s) for ' + id);
      } else {
        console.log('[MediaManager] No cache keys found for ' + id + ' (may already be fresh)');
      }
    } catch (e) {
