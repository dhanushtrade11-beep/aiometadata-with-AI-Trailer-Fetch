...
RETURN: Only the video ID of the best match (11 characters). Nothing else.
...
// Fallback: basic keyword filter if Gemini is unavailable
...
export async function fetchAccurateTrailer(params: TrailerRequest): Promise<string | null> {
  ...
  console.log(`[Trailer] "${title}" | Indian: ${indian} | Fallback: ${fallbackLang}`);

  for (let i = 0; i < queries.length; i++) {
    const query = queries[i];
    console.log(`[Trailer] Query ${i + 1}: "${query}"`);

    let ytId: string | null = null;

    if (youtubeApiKey) {
      // Get real YouTube candidates
      const candidates = await searchYouTubeCandidates(query, youtubeApiKey);
      console.log(`[Trailer] YouTube returned ${candidates.length} candidates`);

      if (candidates.length > 0) {
        if (geminiApiKey) {
          // Best: Gemini picks the most accurate from real YouTube results
      ...
      // No YouTube API: Gemini alone (less accurate, free 1500/day)
      const prompt = `Find the official YouTube trailer ID for: "${query}"
Rules: Official channel only, full trailer only, no teasers/reviews/reactions.
Return ONLY the 11-character YouTube video ID or NULL.`;
    ...
    if (ytId) {
      console.log(`[Trailer] ✓ "${title}" → ${ytId}`);
      return ytId;
    }
  }
