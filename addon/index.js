...
// Body parsers must come BEFORE route registration so POST bodies are available
addon.use(express.json({ limit: '2mb' }));
addon.use(express.urlencoded({ extended: true }));

// registerMediaManagerRoutes called below after redis is loaded
...
// Register media manager routes now that redis client is available
registerMediaManagerRoutes(addon, redis);

// ─── applyOverride (inlined to avoid dist/ path issues) ──────────────────────
const OVERRIDE_PREFIX_LOCAL = 'override:v1:';
...
async function applyOverride(redis, stremioId, meta) {
  ...
  try {
    ...
    // Try all possible ID formats — override may be saved under any of them
    const candidates = new Set();
    if (stremioId)    candidates.add(stremioId);
    if (meta.id)      candidates.add(meta.id);
    if (meta.imdb_id) candidates.add(meta.imdb_id);
    if (meta._imdbId) candidates.add(meta._imdbId);
    if (meta._tmdbId) candidates.add(`tmdb:${meta._tmdbId}`);

    console.log(`[Override] Checking "${meta.name}" with keys:`, [...candidates].join(', '));

    let override = null;
    let matchedKey = null;
    for (const key of candidates) {
      try {
        const raw = await redis.get(`${OVERRIDE_PREFIX_LOCAL}${key}`);
        if (raw) { override = JSON.parse(raw); matchedKey = key; break; }
      } catch {}
    }
    ...
    console.log(`[Override] Found override for "${meta.name}" via key: ${matchedKey}`);
    ...
    if (override.poster) {
      console.log('[Override] Poster: ' + (meta.poster || 'none') + ' -> ' + override.poster);
      meta.poster = override.poster;
    }
...
addon.get("/stremio/:userUUID/catalog/:type/:id{/:extra}.json", async function (req, res) {
  ...
  try {
    ...
    // Apply Media Manager poster overrides to catalog results
    if (responseData && Array.isArray(responseData.metas) && responseData.metas.length > 0 && redis) {
      try {
        for (const meta of responseData.metas) {
          if (!meta || !meta.id) continue;

          // Build all candidate keys — override may be saved under any ID format
          const candidates = new Set();
          candidates.add(meta.id);                                          // e.g. tt1234567
          if (meta.imdb_id)   candidates.add(meta.imdb_id);                // separate imdb_id field
          if (meta._imdbId)   candidates.add(meta._imdbId);
          if (meta._tmdbId)   candidates.add('tmdb:' + meta._tmdbId);      // tmdb:XXXXXX
          // If id is an imdb id, also try tmdb: variants from other fields
          if (meta.id && meta.id.startsWith('tt')) {
            // try tmdb id from meta fields
            if (meta.tmdb_id)  candidates.add('tmdb:' + meta.tmdb_id);
            if (meta._tmdbId)  candidates.add('tmdb:' + meta._tmdbId);
          }
          // If id is tmdb:XXXXX also try numeric only
          if (meta.id && meta.id.startsWith('tmdb:')) {
            candidates.add(meta.id.replace('tmdb:', ''));
          }
          ...
          for (const key of candidates) {
            ...
            try {
              const raw = await redis.get('override:v1:' + key);
              if (raw) {
                const ov = JSON.parse(raw);
                if (ov.poster)     meta.poster     = ov.poster;
                if (ov.background) meta.background = ov.background;
                if (ov.logo)       meta.logo       = ov.logo;
        ...
        console.error('[Catalog Override] Error:', e.message);
      }
    }
...
addon.get("/stremio/:userUUID/meta/:type/:id.json", async function (req, res) {
  ...
  try {
    ...
    let result = await cacheWrapMetaSmart(
      ...
      stremioId,
      async () => {
        return await getMeta(type, language, stremioId, fullConfig, userUUID, true, true);
      },
      undefined,
      cacheOptions,
      type,
    ...
    if (!result || !result.meta) {
      ...
      if (canonicalFallbackId) {
        ...
        result = await cacheWrapMetaSmart(
          ...
          canonicalFallbackId,
          async () => {
            return await getMeta(type, language, canonicalFallbackId, fullConfig, userUUID, true, true);
          },
          undefined,
          cacheOptions,
          type,
...
// ─── Site Password Protection (Cookie Session) ───────────────────────────────
// Set SITE_USERNAME and SITE_PASSWORD in Render environment variables.
// Once logged in, a signed cookie keeps you authenticated — no repeated prompts.
// Stremio addon routes are always public so the addon works in Stremio.

const SITE_USERNAME = process.env.SITE_USERNAME;
const SITE_PASSWORD = process.env.SITE_PASSWORD;
const COOKIE_SECRET = process.env.COOKIE_SECRET || process.env.SITE_PASSWORD || 'changeme-set-COOKIE_SECRET';
const COOKIE_NAME   = 'aio_auth';
const COOKIE_MAXAGE = 30 * 24 * 60 * 60 * 1000; // 30 days

// Simple HMAC-signed cookie (no extra packages needed)
function signCookie(value) {
  const hmac = require('crypto').createHmac('sha256', COOKIE_SECRET);
  hmac.update(value);
...
function verifyCookie(signed) {
  ...
  return signed === expected && value === 'authenticated';
}
...
// Login page HTML
const LOGIN_HTML_PARTS = [
  '<!DOCTYPE html><html lang="en"><head>',
  '<meta charset="UTF-8">',
  '<meta name="viewport" content="width=device-width, initial-scale=1.0">',
  '<title>AIOMetadata - Login</title>',
  '<style>',
  ...
  '.card{background:#1a1d27;border:1px solid #2a2d3a;border-radius:16px;padding:40px 32px;width:100%;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,0.5)}',
  'h1{color:#fff;font-size:22px;font-weight:700;text-align:center;margin-bottom:8px}',
  'p{color:#6b7280;font-size:14px;text-align:center;margin-bottom:28px}',
  'label{display:block;color:#9ca3af;font-size:13px;font-weight:500;margin-bottom:6px}',
  'input{width:100%;background:#0f1117;border:1px solid #2a2d3a;border-radius:8px;color:#fff;font-size:15px;padding:12px 14px;outline:none;transition:border-color .2s;margin-bottom:16px}',
  'input:focus{border-color:#3b82f6}',
  'button{width:100%;background:#3b82f6;color:#fff;font-size:15px;font-weight:600;border:none;border-radius:8px;padding:13px;cursor:pointer;transition:background .2s;margin-top:4px}',
  'button:hover{background:#2563eb}',
  '.error{background:#7f1d1d;border:1px solid #ef4444;border-radius:8px;color:#fca5a5;font-size:13px;padding:10px 14px;margin-bottom:16px;text-align:center}',
  '</style></head><body>',
  '<div class="card">',
  '<h1>AIOMetadata</h1>',
  '<p>Private instance - enter credentials to continue</p>',
  '{{ERROR}}',
...
function buildLoginPage(error) {
  var html = LOGIN_HTML_PARTS.join('');
  return html.replace('{{ERROR}}', error || '');
}

if (SITE_USERNAME && SITE_PASSWORD) {
  // Handle login POST — use req.body (already parsed by express.urlencoded)
  addon.post('/auth/login', function(req, res) {
    var user = (req.body && req.body.username) || '';
    var pass = (req.body && req.body.password) || '';
    var redirectTo = (req.body && req.body.next) || req.query.next || '/configure';
    if (user === SITE_USERNAME && pass === SITE_PASSWORD) {
      var signed = signCookie('authenticated');
      var cookieAge = Math.floor(COOKIE_MAXAGE / 1000);
      res.setHeader('Set-Cookie', COOKIE_NAME + '=' + encodeURIComponent(signed) + '; Path=/; HttpOnly; SameSite=Lax; Max-Age=' + cookieAge);
      return res.redirect(302, redirectTo);
    }
    res.status(401).send(buildLoginPage('<div class="error">Invalid username or password</div>'));
  });

  addon.get('/auth/login', function(req, res) {
    var nextField = req.query.next
      ? '<input type="hidden" name="next" value="' + req.query.next + '" />'
      : '';
    var html = buildLoginPage('').replace('</form>', nextField + '</form>');
    res.send(html);
  });

  addon.get('/auth/logout', function(req, res) {
    res.setHeader('Set-Cookie', COOKIE_NAME + '=; Path=/; HttpOnly; Max-Age=0');
    res.redirect('/auth/login');
  });

  var authGuard = function(req, res, next) {
    var ALWAYS_PUBLIC = ['/stremio/', '/catalog/', '/meta/', '/stream/', '/auth/', '/favicon', '/manifest.json'];
    var isPublic = ALWAYS_PUBLIC.some(function(p) { return req.path === p || req.path.startsWith(p); });
    if (isPublic) return next();
    var cookies = parseCookies(req);
    var cookieVal = cookies[COOKIE_NAME] ? decodeURIComponent(cookies[COOKIE_NAME]) : null;
    if (verifyCookie(cookieVal)) return next();
    var next_url = encodeURIComponent(req.originalUrl);
    res.redirect(302, '/auth/login?next=' + next_url);
    // next is read from req.query.next in the GET handler and injected as hidden field
  };

  addon.use(authGuard);
  console.log('[Auth] Cookie session protection enabled - login once, stay logged in for 30 days');
} else {
  console.log('[Auth] No SITE_USERNAME/SITE_PASSWORD set - site is public');
}
// ─────────────────────────────────────────────────────────────────────────────
