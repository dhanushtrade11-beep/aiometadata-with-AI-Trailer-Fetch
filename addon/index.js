...
// Set SITE_USERNAME and SITE_PASSWORD in Render environment variables to protect
// the entire configure page with HTTP Basic Auth.
// Stremio addon routes (/stremio/, /manifest.json, /catalog/, /meta/, /stream/)
// are always public so the addon keeps working in Stremio.
const SITE_USERNAME = process.env.SITE_USERNAME;
const SITE_PASSWORD = process.env.SITE_PASSWORD;

if (SITE_USERNAME && SITE_PASSWORD) {
  const basicAuthCheck = (req, res, next) => {
    // Always allow Stremio addon routes and API routes
    const PUBLIC_PATHS = ['/stremio/', '/manifest.json', '/catalog/', '/meta/', '/stream/', '/api/auth/', '/favicon'];
    const isPublic = PUBLIC_PATHS.some(p => req.path.startsWith(p)) || req.path === '/manifest.json';
    ...
    // Not authenticated — ask browser to show login dialog
    res.setHeader('WWW-Authenticate', 'Basic realm="AIOMetadata - Private"');
    res.status(401).send('Authentication required. Please enter your username and password.');
  };

  addon.use(basicAuthCheck);
  console.log('[Auth] Site password protection enabled');
} else {
