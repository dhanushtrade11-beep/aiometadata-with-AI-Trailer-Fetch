...
if (SITE_USERNAME && SITE_PASSWORD) {
  // Parse body for login form (simple urlencoded)
  addon.use('/auth/login', (req, res, next) => {
    ...
    req.on('end', () => {
      ...
      if (user === SITE_USERNAME && pass === SITE_PASSWORD) {
        ...
        const redirectTo = params.get('next') || '/configure';
        res.setHeader('Set-Cookie', \`\${COOKIE_NAME}=\${encodeURIComponent(signed)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=\${COOKIE_MAXAGE / 1000}\`);
  ...
  addon.get('/auth/login', (req, res) => {
    res.send(LOGIN_HTML.replace('{{ERROR}}', ''));
  });

  addon.get('/auth/logout', (req, res) => {
  ...
  const authGuard = (req, res, next) => {
    // Always allow: Stremio routes, API health, login page itself
    const ALWAYS_PUBLIC = ['/stremio/', '/catalog/', '/meta/', '/stream/', '/auth/', '/favicon', '/manifest.json'];
    const isPublic = ALWAYS_PUBLIC.some(p => req.path === p || req.path.startsWith(p));
    if (isPublic) return next();

    const cookies = parseCookies(req);
    const cookieVal = cookies[COOKIE_NAME] ? decodeURIComponent(cookies[COOKIE_NAME]) : null;
